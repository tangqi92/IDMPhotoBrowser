...
- (CGRect)frameForDownloadButtonAtOrientation:(UIInterfaceOrientation)orientation;
...
// Properties
@synthesize displayDoneButton = _displayDoneButton, displayToolbar = _displayToolbar, displayActionButton = _displayActionButton, displayCounterLabel = _displayCounterLabel, useWhiteBackgroundColor = _useWhiteBackgroundColor, doneButtonImage = _doneButtonImage, displayDownloadButton = _displayDownloadButton, downloadButtonImage = _downloadButtonImage;
...
- (void)viewDidLoad {
    ...
    [_downloadButton setFrame:[self frameForDownloadButtonAtOrientation:currentOrientation]];
    ...
    if (!_downloadButtonImage) {
        [_downloadButton setTitleColor:[UIColor colorWithWhite:0.9 alpha:0.9] forState:UIControlStateNormal|UIControlStateHighlighted];
        [_downloadButton setTitle:IDMPhotoBrowserLocalizedStrings(@"Download") forState:UIControlStateNormal];
        [_downloadButton.titleLabel setFont:[UIFont boldSystemFontOfSize:11.0f]];
        [_downloadButton setBackgroundColor:[UIColor colorWithWhite:0.1 alpha:0.5]];
        _downloadButton.layer.cornerRadius = 3.0f;
        _downloadButton.layer.borderColor = [UIColor colorWithWhite:0.9 alpha:0.9].CGColor;
        _downloadButton.layer.borderWidth = 1.0f;
    } else {
        [_downloadButton setImage:_downloadButtonImage forState:UIControlStateNormal];
...
- (void)hideControlsAfterDelay {
    ...
    if (![self areControlsHidden]) {
        [self cancelControlHiding];
        _controlVisibilityTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(hideControls) userInfo:nil repeats:NO];
    }
}
